...
export default {
  ...
  methods: {
    getDetails (name) {
      return this.inventory.find((el, i) => {
        return el.name === name
      })
    },
    getPrice (name) {
      // const product = this.getDetails(name);
    ...
    calculateTotal () {
      ...
      const total = Object.values(this.cart).reduce((acc, curr, i) => {
        return acc + curr * this.getPrice(names[i])
      }, 0)
